...
void timeAddArr(long double** timeArr, int timeSize);
...
int main()
{
	...
	long double** voteArr = new long double* [size];		// Arr with number of vote
	timeAddArr(voteArr, size);
	readallfile(name, arrSize, number, countryArr, voteArr, ans2);
	showArrD(voteArr, size);
...
void timeAddArr(long double** timeArr, int timeSize) {		
	for (int i = 0; i < timeSize; i++) {
		timeArr[i] = new long double[timeSize];
	}
}
